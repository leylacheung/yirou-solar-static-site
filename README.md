...
- 当前目录共 32 款产品，只分为高性能材料、并噻吩中间体、端基三类；分类数量分别为 18、6、8。
- `productDefinitions` 保存产品名称、分类和结构图文件名，也是网站允许展示的产品清单。
- `priceEntries` 保存产品分类、批次、规格和人民币价格。
- 价格数字直接写整数，例如 `2500` 会显示为 `¥2,500`。
- 每个价格都是含税、含运费参考价；最终报价说明不要删除。

只有以下三组会自动合并：`PNDIT-F3N / F3N`、`BTP-eC9 / BTP-Ec9`、`PYVR / PY-V-r`。新增别名时必须先确认确实为同一产品。

## 替换产品结构图

结构图位于 `images/products`。新图片建议使用白色背景 PNG。productDefinitions 的第三个字段只填写 PNG 文件名，例如 foo.png；不能填写 images/products/ 前缀。实际图片文件放到 images/products/foo.png。替换后刷新浏览器即可看到新图片。

Q1200、S1300、H3130 当前没有结构图，页面会显示“结构暂未提供”。不要为这三款产品借用其他结构图；取得准确资料后，再把文件加入 `images/products` 并更新 `productDefinitions`。

## 替换微信二维码

微信二维码位于 `images/contact/wechat-qr.jpg`。替换时保留相同文件名和 JPG 格式，并保留二维码四周的白色空白区域，避免影响扫码识别。
