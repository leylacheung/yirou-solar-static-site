(() => {
  ...
  const structures = [
    ...
    ["N3", ["高性能材料"], "n3.png"],
    ["PNDIT-F3N", ["高性能材料"], "f3n-pndit-f3n.png"],
  ...
  function buildProducts() {
    ...
    for (const [sourceName, categories, image] of structures) {
      const product = ensure(sourceName);
      product.image = `images/products/${image}`;
      for (const category of categories) if (!product.categories.includes(category)) product.categories.push(category);
    }

    for (const [sourceName, category, groups] of priceEntries) {
      const product = ensure(sourceName);
  ...
  function productCardMarkup(product) {
    const image = product.image
      ...
      : '<p class="product-card__missing">结构资料待补充</p>';
    ...
    return `<article class="product-card"><div class="product-card__tags">${product.categories.map((category) => `<span>${escapeHtml(category)}</span>`).join("")}</div><h3>${escapeHtml(product.name)}</h3>${product.aliases.length ? `<p class="product-card__aliases">别名：${product.aliases.map(escapeHtml).join(" / ")}</p>` : ""}<div class="product-card__structure">${image}</div><div class="product-card__prices">${prices}</div><p class="product-card__tax">含税、含运费</p></article>`;
  }
